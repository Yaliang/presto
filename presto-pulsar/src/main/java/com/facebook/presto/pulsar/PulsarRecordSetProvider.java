...
import static com.facebook.presto.pulsar.PulsarHandleResolver.convertSplit;
import static java.util.Objects.requireNonNull;

/**
 * Factory for Pulsar specific {@link RecordSet} instances.
 */
        ...
        implements ConnectorRecordSetProvider
{
    ...
    {
        PulsarSplit pulsarSplit = convertSplit(split);
        ...
        RowDecoder keyDecoder = registry.getRowDecoder(pulsarSplit.getKeyDataFormat());
        RowDecoder messageDecoder = registry.getRowDecoder(pulsarSplit.getMessageDataFormat());

        for (ColumnHandle handle : columns) {
            PulsarColumnHandle columnHandle = convertColumnHandle(handle);
            handleBuilder.add(columnHandle);

            if (!columnHandle.isInternal()) {
                if (columnHandle.isKeyDecoder()) {
                    FieldDecoder<?> fieldDecoder = registry.getFieldDecoder(
                            pulsarSplit.getKeyDataFormat(),
                    ...
                    FieldDecoder<?> fieldDecoder = registry.getFieldDecoder(
                            pulsarSplit.getMessageDataFormat(),
                            columnHandle.getType().getJavaType(),
                            columnHandle.getDataFormat());
