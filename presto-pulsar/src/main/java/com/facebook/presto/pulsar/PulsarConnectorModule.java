...
/**
 * Guice module for the Apache Pulsar connector.
 */
        ...
        implements Module
{
    ...
    public void configure(Binder binder)
    {
        ...
        jsonCodecBinder(binder).bindJsonCodec(PulsarTopicDescription.class);

        binder.install(new DecoderModule());

        for (PulsarInternalFieldDescription internalFieldDescription : PulsarInternalFieldDescription.getInternalFields()) {
            bindInternalColumn(binder, internalFieldDescription);
        }
    }

    private static void bindInternalColumn(Binder binder, PulsarInternalFieldDescription fieldDescription)
    {
        Multibinder<PulsarInternalFieldDescription> fieldDescriptionBinder = Multibinder.newSetBinder(binder, PulsarInternalFieldDescription.class);
